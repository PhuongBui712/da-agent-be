    ...
    return {
        ...
        "reporter": AgentDefinition(
            ...
            prompt=(
                ...
                "\n\n"
                "**Working-directory discipline.** The dispatch prompt names "
                "`working_dir=...` and `output_path=...`. Treat `working_dir` as "
                "your scratch root for intermediate files (.py scripts, .csv "
                "exports, .png chart drafts, debug logs). Treat `output_path` as the "
                "FINAL deliverable target — write to it verbatim, never to a sibling "
                "path. Never write to /tmp, never under outputs/<sid>/ except via "
                "`output_path` verbatim, never to a path you invent.\n\n"
                "**`output_path` is writable.** The harness has already mounted "
                "the parent directory of `output_path` into the sandbox before "
                "you start. Open it directly with whatever library is "
                "appropriate (`prs.save(output_path)`, "
                "`workbook.save(output_path)`, `with open(output_path, 'wb')`). "
                "If your first save fails, FIX THE PYTHON ERROR — DO NOT fall "
                "back to writing into `working_dir` and trying to copy / "
                "symlink / hard-link the file to `output_path` afterwards. "
                "The path is reachable; spurious EROFS / EXDEV errors mean a "
                "code bug (typo in the path, wrong permission flag, leftover "
                "tempfile collision), not a sandbox restriction.\n\n"
                "**Language preservation.** The dispatch prompt names a target "
                "language (or implies it from the user request). Write all "
                "user-visible text — slide titles, slide bodies, doc paragraphs, "
                "sheet headers, chart labels — in that language. When the language "
                "is Vietnamese, preserve EVERY diacritic verbatim (`Ngân hàng`, "
                "never `Ngan hang`; `chó`, never `cho`; `Tạo`, never `Tao`). Do "
                "NOT transliterate. Do NOT strip tone marks. Do NOT normalise to "
                "ASCII. The user prompt body forwarded in the dispatch is the "
                "authoritative source for spelling."
            ),
            tools=_READWRITE,
